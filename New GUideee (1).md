# A1-G3 AWS Cloud Infrastructure - Complete Deployment Plan
...
All resources follow the `A1-G3-` prefix for consistent identification and management.


| Resource Type                | AWS Console Name             | Purpose                        |
| ---------------------------- | ---------------------------- | ------------------------------ |
| **Network Foundation**       |                              |                                |
| VPC                          | A1-G3-VPC                    | Primary virtual network        |
| Internet Gateway             | A1-G3-IGW                    | Outbound internet access       |
| NAT Gateway                  | A1-G3-NAT                    | Private subnet internet access |
| **Subnets**                  |                              |                                |
| Public Subnet AZ1            | A1-G3-Public-Subnet-AZ1      | ALB and NAT (us-east-1a)       |
| Public Subnet AZ2            | A1-G3-Public-Subnet-AZ2      | ALB redundancy (us-east-1b)    |
| Private EC2 Subnet AZ1       | A1-G3-Private-EC2-Subnet-AZ1 | Web servers (us-east-1a)       |
| Private EC2 Subnet AZ2       | A1-G3-Private-EC2-Subnet-AZ2 | Web servers (us-east-1b)       |
| Private RDS Subnet AZ1       | A1-G3-Private-RDS-Subnet-AZ1 | Database primary (us-east-1a)  |
| Private RDS Subnet AZ2       | A1-G3-Private-RDS-Subnet-AZ2 | Database standby (us-east-1b)  |
| **Routing**                  |                              |                                |
| Public Route Table           | A1-G3-Public-RT              | Routes to Internet Gateway     |
| Private Route Table          | A1-G3-Private-RT             | Routes to NAT Gateway          |
| **Security**                 |                              |                                |
| ALB Security Group           | A1-G3-ALB-SG                 | Load balancer firewall         |
| EC2 Security Group           | A1-G3-EC2-SG                 | Web server firewall            |
| RDS Security Group           | A1-G3-RDS-SG                 | Database firewall              |
| IAM Role                     | Inventory-App-Role           | EC2 parameter access           |
| **Compute**                  |                              |                                |
| EC2 Instance AZ1             | A1-G3-EC2-AZ1                | Initial web server             |
| EC2 Instance AZ2             | A1-G3-EC2-AZ2                | ASG-launched instances         |
| AMI                          | A1-G3-AMI                    | Golden image                   |
| Launch Template              | A1-G3-LT                     | ASG configuration              |
| **Database**                 |                              |                                |
| RDS Instance                 | A1-G3-RDS                    | MySQL database                 |
| DB Subnet Group              | A1-G3-DB-Subnet-Group        | RDS network placement          |
| **Load Balancing & Scaling** |                              |                                |
| Target Group                 | A1-G3-TG                     | EC2 health monitoring          |
| Application Load Balancer    | A1-G3-ALB                    | Traffic distribution           |
| Auto Scaling Group           | A1-G3-ASG                    | Dynamic scaling                |
...
| Name tag            | `A1-G3-VPC`        | Exactly as shown      |
...
2. Wait for success message: "Successfully created vpc-xxxxxx"

**Verification:**

- VPC list shows A1-G3-VPC with State = Available
- CIDR shows 10.0.0.0/16

📸 **Screenshot Required:** VPC dashboard showing A1-G3-VPC with CIDR 10.0.0.0/16
...
2. You'll see: "Successfully created igw-xxxxxx"
3. The page shows the newly created IGW with State = Detached
4. Click **Actions** (top right) → **Attach to VPC**
...
| Field          | Value                            |
| -------------- | -------------------------------- |
| Available VPCs | Select `A1-G3-VPC` from dropdown |


1. Click **Attach internet gateway**
2. Success message: "Internet Gateway igw-xxxxxx was attached to VPC vpc-xxxxxx"

**Verification:**

- State changes from "Detached" to "Attached"
- VPC ID column shows A1-G3-VPC

📸 **Screenshot Required:** Internet Gateway showing State = Attached to A1-G3-VPC
...
| Field  | Value                            |
| ------ | -------------------------------- |
| VPC ID | Select `A1-G3-VPC` from dropdown |
...
**For A1-G3-Public-Subnet-AZ1:**

1. In subnet list, find and **select** A1-G3-Public-Subnet-AZ1 (checkbox on left)
2. Click **Actions** → **Edit subnet settings**
3. Under "Auto-assign IP settings":
  - ✅ Check **Enable auto-assign public IPv4 address**
4. Click **Save**

**For A1-G3-Public-Subnet-AZ2:**

1. Select A1-G3-Public-Subnet-AZ2
2. Click **Actions** → **Edit subnet settings**
...
| Field                    | Value                        | Notes                        |
| ------------------------ | ---------------------------- | ---------------------------- |
| Name                     | `A1-G3-NAT`                  | Optional but recommended     |
| Subnet                   | `A1-G3-Public-Subnet-AZ1`    | **MUST be a public subnet**  |
...
2. Success message appears with NAT Gateway ID
...
| Field | Value                            |
| ----- | -------------------------------- |
| Name  | `A1-G3-Public-RT`                |
| VPC   | Select `A1-G3-VPC` from dropdown |


1. Click **Create route table**
2. Success message: "Route table created successfully"

**Add Internet Gateway Route:**

The newly created route table is automatically displayed. If not, click on A1-G3-Public-RT.

1. Bottom panel shows tabs: **Routes | Subnet associations | Edge associations**
2. Click on **Routes** tab
3. Click **Edit routes** (button on right)
4. Click **Add route**


| Destination | Target                                              |
| ----------- | --------------------------------------------------- |
| `0.0.0.0/0` | Select**Internet Gateway**, then select `A1-G3-IGW` |
...
2. Under "Explicit subnet associations" section, click **Edit subnet associations**
3. Select (checkbox) both:
  - ✅ A1-G3-Public-Subnet-AZ1
    - ✅ A1-G3-Public-Subnet-AZ2
...
- Routes tab shows:
  - 10.0.0.0/16 → local
  - 0.0.0.0/0 → igw-xxxxx (A1-G3-IGW)
- Subnet associations shows 2 subnets associated

📸 **Screenshot Required:** A1-G3-Public-RT showing routes and subnet associations
...
| Field | Value              |
| ----- | ------------------ |
| Name  | `A1-G3-Private-RT` |
| VPC   | Select `A1-G3-VPC` |
...
| Destination | Target                                         |
| ----------- | ---------------------------------------------- |
| `0.0.0.0/0` | Select**NAT Gateway**, then select `A1-G3-NAT` |
...
3. Select (checkbox) both:
  - ✅ A1-G3-Private-EC2-Subnet-AZ1
  - ✅ A1-G3-Private-EC2-Subnet-AZ2
4. **DO NOT select** A1-G3-Private-RDS-Subnet-AZ1 or A1-G3-Private-RDS-Subnet-AZ2
...
📸 **Screenshot Required:** A1-G3-Private-RT showing NAT Gateway route and EC2 subnet associations
...
```
Internet → A1-G3-ALB-SG → A1-G3-EC2-SG → A1-G3-RDS-SG
```
...
| Field               | Value                                          |
| ------------------- | ---------------------------------------------- |
| Security group name | `A1-G3-ALB-SG`                                 |
...
📸 **Screenshot Required:** A1-G3-ALB-SG inbound rules showing HTTP from 0.0.0.0/0
...
| Field               | Value                                |
| ------------------- | ------------------------------------ |
| Security group name | `A1-G3-EC2-SG`                       |
...
- In the text field that appears, type `A1-G3-ALB-SG`
- Select the security group that appears: "A1-G3-ALB-SG (sg-xxxxx)"
...
📸 **Screenshot Required:** A1-G3-EC2-SG inbound rules showing:

- HTTP from A1-G3-ALB-SG
- SSH from your IP
...
| Field               | Value                                   |
| ------------------- | --------------------------------------- |
| Security group name | `A1-G3-RDS-SG`                          |
...
- In text field, type `A1-G3-EC2-SG`
- Select "A1-G3-EC2-SG (sg-xxxxx)" from dropdown
...
| -------------- | ------------------------------------------------ |
| A1-G3-ALB-SG   | HTTP (80) from 0.0.0.0/0                         |
| A1-G3-EC2-SG   | HTTP (80) from A1-G3-ALB-SG, SSH (22) from My IP |
| A1-G3-RDS-SG   | MySQL (3306) from A1-G3-EC2-SG                   |


📸 **Screenshot Required:** A1-G3-RDS-SG inbound rules showing MySQL from A1-G3-EC2-SG
...
- A1-G3-VPC created (10.0.0.0/16)
- A1-G3-IGW attached to A1-G3-VPC
- All 6 subnets created with correct names, AZs, and CIDRs
- Public subnets have auto-assign public IPv4 enabled
- A1-G3-NAT in A1-G3-Public-Subnet-AZ1, State = Available
- A1-G3-Public-RT routes 0.0.0.0/0 to IGW
- A1-G3-Public-RT associated with both public subnets
- A1-G3-Private-RT routes 0.0.0.0/0 to NAT
- A1-G3-Private-RT associated with both EC2 private subnets only
- A1-G3-ALB-SG allows HTTP from 0.0.0.0/0
- A1-G3-EC2-SG allows HTTP from A1-G3-ALB-SG and SSH from My IP
- A1-G3-RDS-SG allows MySQL from A1-G3-EC2-SG
- Inventory-App-Role verified with SSM access
- All required screenshots captured

### Screenshots Checklist for Member 3:

1. ✅ A1-G3-VPC dashboard with CIDR
2. ✅ A1-G3-IGW showing State = Attached
3. ✅ All 6 subnets with names and CIDRs
4. ✅ A1-G3-NAT showing Available state and Elastic IP
5. ✅ A1-G3-Public-RT routes and subnet associations
6. ✅ A1-G3-Private-RT routes and subnet associations
7. ✅ A1-G3-ALB-SG inbound rules
8. ✅ A1-G3-EC2-SG inbound rules
9. ✅ A1-G3-RDS-SG inbound rules
...
| -------------------------- | ------------------------------------- |
| VPC                        | Select `A1-G3-VPC`                    |
| Subnet                     | Select `A1-G3-Private-EC2-Subnet-AZ1` |
| Auto-assign public IP      | **Disable**                           |
| Firewall (security groups) | Select existing security group        |
| Common security groups     | Select `A1-G3-EC2-SG`                 |
...
- Instance type: t2.small
- Subnet: A1-G3-Private-EC2-Subnet-AZ1
- Security group: A1-G3-EC2-SG
...
**Start session:**
4. You should see A1-G3-EC2-AZ1 in the list of available instances

- If you don't see it, wait 1-2 minutes and refresh - the SSM agent needs time to register

1. Select the radio button next to **A1-G3-EC2-AZ1**
2. Click **Start session** (orange button, bottom right)
...
4. In the Value field, replace "PLACEHOLDER" with the RDS endpoint URL Member 2 provides
  - It will look like: `A1-G3-rds.xxxxxxxxx.us-east-1.rds.amazonaws.com`
...
- A1-G3-EC2-AZ1 launched and running
- Instance is in A1-G3-Private-EC2-Subnet-AZ1
...
1. ✅ A1-G3-EC2-AZ1 instance details
...
| Field       | Value                                          |
| ----------- | ---------------------------------------------- |
| Name        | `A1-G3-DB-Subnet-Group`                        |
| Description | `RDS subnets for A1-G3 project across two AZs` |
| VPC         | Select `A1-G3-VPC`                             |
...
| Field                   | Value              |
| ----------------------- | ------------------ |
| DB instance identifier  | `A1-G3-RDS`        |
...
| Field                        | Value                                |
| ---------------------------- | ------------------------------------ |
| Virtual private cloud (VPC)  | Select `A1-G3-VPC`                   |
| DB subnet group              | Select `A1-G3-DB-Subnet-Group`       |
| Public access                | **No**                               |
| VPC security group           | Choose existing                      |
| Existing VPC security groups | Remove "default", add `A1-G3-RDS-SG` |
...
3. In the dropdown, search and select `A1-G3-RDS-SG`
...
"Successfully created database A1-G3-RDS"
...
Click on `A1-G3-RDS` to view details
...
| VPC security groups | A1-G3-RDS-SG (active) |
| Subnet group        | A1-G3-DB-Subnet-Group |
| Availability Zone   | us-east-1a            |
...
- Find **Endpoint** field
- Click the copy icon
- It looks like: `A1-G3-rds.c1234567890.us-east-1.rds.amazonaws.com`
...
#### Step 3.2 - Locate the SQL Dump File on EC2

Since you pushed your `Countrydatadump.sql` file to your GitHub repo and ran the `git clone` command in Task 4, the file is already on your server!
...
Expected output showing the file size (should be around 130KB - 150KB).
...
Connect to MySQL (replace **YOUR-RDS-ENDPOINT** with your actual RDS host):

```bash
# PASTE YOUR ENDPOINT HERE:
mysql -h YOUR-RDS-ENDPOINT-HERE -u admin -p
```

*Example: mysql -h A1-G3-rds.c123...us-east-1.rds.amazonaws.com -u admin -p*
...
Replace with your actual RDS endpoint: `A1-G3-rds.c0bortbeujiu.us-east-1.rds.amazonaws.com`
...
> A1-G3-rds.c1234567890.us-east-1.rds.amazonaws.com
>
...
- A1-G3-DB-Subnet-Group created with both RDS subnets
- A1-G3-RDS instance created with Production template
- Multi-AZ = Yes
- Publicly accessible = No
- Security group = A1-G3-RDS-SG
...
1. ✅ A1-G3-DB-Subnet-Group details
2. ✅ A1-G3-RDS showing Available, Multi-AZ, Not publicly accessible
3. ✅ RDS endpoint URL
...
- Member 1 has fully configured A1-G3-EC2-AZ1 with PHP application
...
3. Find and select **A1-G3-EC2-AZ1** (checkbox on left)
...
📸 **Screenshot Required:** AMI list showing A1-G3-AMI with Status = Available
...
| Field                        | Value                                 |
| ---------------------------- | ------------------------------------- |
| Launch template name         | `A1-G3-LT`                            |
| Template version description | `v1 - Initial template for A1-G3 ASG` |
...
| -------------------------- | ------------------ |
| Source                     | **My AMIs** (tab)  |
| Amazon Machine Image (AMI) | Select `A1-G3-AMI` |


You should see: "A1-G3-AMI (ami-xxxxxx)"
...
| Firewall (security groups) | Select existing security group   |
| Security groups            | Select `A1-G3-EC2-SG`            |
...
| ------ | --------------- | ---------------------- |
| `Name` | `A1-G3-EC2-AZ2` | ✅ Instances, ✅ Volumes |
...
Success message: "Successfully created A1-G3-LT"

Click **View launch template**

**Verification:**

- Template name: A1-G3-LT
- Latest version: 1
- AMI: A1-G3-AMI (ami-xxxxxx)
- Instance type: t2.small
- Security group: A1-G3-EC2-SG
- IAM role: Inventory-App-Role
- Tags: Name=A1-G3-EC2-AZ2
...
| Select security groups | Select `A1-G3-ALB-SG` |
...
| Protocol       | HTTP                  |
| Port           | 80                    |
| Default action | Forward to `A1-G3-TG` |


Click on the "Forward to" dropdown and select `A1-G3-TG`
...
- Security group: A1-G3-ALB-SG
- Listener: HTTP:80 → A1-G3-TG
...
| Field                   | Value             |
| ----------------------- | ----------------- |
| Auto Scaling group name | `A1-G3-ASG`       |
| Launch template         | Select `A1-G3-LT` |
...
- Existing load balancer target groups: Select `A1-G3-TG`
...
| Field               | Value                   |
| ------------------- | ----------------------- |
| Scaling policy name | `A1-G3-CPU-Policy`      |
...
Tags are already configured in the Launch Template (Name=A1-G3-EC2-AZ2).
...
- Name: A1-G3-ASG
- Launch template: A1-G3-LT
- VPC: A1-G3-VPC
- Subnets: 2 private EC2 subnets (AZ1 and AZ2)
- Load balancer: A1-G3-TG
...
Go to Auto Scaling Groups → A1-G3-ASG → Activity tab
...
| Instance names          | A1-G3-EC2-AZ1, A1-G3-EC2-AZ2 (×2)             |
...
1. EC2 → Auto Scaling Groups → A1-G3-ASG
...
1. Select one of the ASG-launched instances (A1-G3-EC2-AZ2)
...
1. EC2 → Target Groups → A1-G3-TG → Targets tab
...
1. EC2 → Auto Scaling Groups → A1-G3-ASG → Activity tab
...
Reason: Monitor "A1-G3-CPU-Policy" triggered a scale-out activity.
```
...
- A1-G3-AMI created and available
- A1-G3-LT created with correct AMI, instance type, security group, IAM role
- Launch template has Name tag for instances
- A1-G3-TG created with /index.php health check
- A1-G3-ALB created, internet-facing, across 2 public subnets
- ALB state = Active
- A1-G3-ASG created with min=1, desired=2, max=4
- ASG uses both private EC2 subnets
- ASG attached to A1-G3-TG
...
1. ✅ A1-G3-AMI showing Available status
2. ✅ A1-G3-LT details (AMI, instance type, SG, IAM role)
3. ✅ A1-G3-TG health check configuration
4. ✅ A1-G3-ALB showing Active, DNS name, 2 AZs
5. ✅ A1-G3-ASG configuration (capacity, subnets, policy)
...
| ------------------ | ------------------------------ | ------------------------------ |
| Public Subnet      | ✅ A1-G3-Public-Subnet-AZ1      | ✅ A1-G3-Public-Subnet-AZ2      |
| Private EC2 Subnet | ✅ A1-G3-Private-EC2-Subnet-AZ1 | ✅ A1-G3-Private-EC2-Subnet-AZ2 |
| Private RDS Subnet | ✅ A1-G3-Private-RDS-Subnet-AZ1 | ✅ A1-G3-Private-RDS-Subnet-AZ2 |
...
```
Internet → A1-G3-ALB-SG (port 80 from 0.0.0.0/0)
           ↓
         A1-G3-EC2-SG (port 80 from A1-G3-ALB-SG only)
           ↓
         A1-G3-RDS-SG (port 3306 from A1-G3-EC2-SG only)
```
...
```bash
mysql -h A1-G3-rds.xxxxxx.us-east-1.rds.amazonaws.com -u admin -p
```
...
| ALB shows no targets              | Target Group not attached        | Verify ASG is attached to A1-G3-TG                          |
...
**Decision:** Single NAT Gateway in A1-G3-Public-Subnet-AZ1 only
...
Internet
   ↓
A1-G3-ALB-SG (Allow: HTTP/80 from 0.0.0.0/0)
   ↓
A1-G3-EC2-SG (Allow: HTTP/80 from A1-G3-ALB-SG, SSH/22 from Admin IP)
   ↓
A1-G3-RDS-SG (Allow: MySQL/3306 from A1-G3-EC2-SG)
```
...
2. Attacker attempts: `aws rds delete-db-instance --db-instance-identifier A1-G3-RDS`
...
```
1. Launch A1-G3-EC2-AZ1
2. Install Apache + PHP
3. Deploy application files
4. Test thoroughly
5. Create A1-G3-AMI (snapshot)
...
- Version AMIs (A1-G3-AMI-v1, v2, v3)
...
- ✅ All resources have systematic names (A1-G3-*)
...
| ------- | ---- | ----------- | -------------------------- |
| 1.0     | 2025 | Group A1-G3 | Initial comprehensive plan |
